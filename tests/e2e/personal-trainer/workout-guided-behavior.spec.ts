...
test.describe('active workout guided behavior', () => {
  ...
  test('minimize saves open inputs, rest continues, and resume reloads the saved workout', async ({ browser }) => {
    ...
    try {
      ...
      await expect(dock.getByText('QA Snapshot Squat 47391', { exact: true })).toBeVisible()
      await expect(dock.getByText(/set 1 of 1/i)).toBeVisible()
      await expect(dock.getByText(/67 kg × 9/i)).toBeVisible()
      await dock.getByRole('button', { name: /collapse minimized workout/i }).click()
      await expect(dock.getByRole('button', { name: /expand minimized workout/i })).toBeVisible()
      await dock.getByRole('button', { name: /expand minimized workout/i }).click()
