-- Combined SQL Editor runner: Phase 22 + Phase 23.
-- Run the entire file once. The transaction is all-or-nothing.
-- Prerequisite: migrations through Phase 21 have already been applied.
...
-- Persist pending/completed state for every workout set.
-- Existing rows default to completed to preserve historical workout records;
-- new application snapshots always send the explicit state.
...
begin
  ...
  insert into public.sets (
    ...
    distance, rest_seconds, difficulty, note, is_completed
  )
  select
    p_workout_id,
    v_actor,
    ...
    coalesce((item->>'is_completed')::boolean, true)
...
select
  exists (
    ...
    where table_schema = 'public' and table_name = 'sets' and column_name = 'is_completed'
...
-- ============================================================================
-- Phase 23: flexible weekly plans and private daily readiness
-- The standalone migration's BEGIN/COMMIT are intentionally owned by this file.
-- ============================================================================
