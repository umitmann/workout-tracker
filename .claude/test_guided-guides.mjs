...
test('single-set and whole-exercise guidance both wire optional speech and pause/play controls', async () => {
  ...
  assert.match(logger, /wt\.tempoStartPhase/)
  assert.match(logger, /aria-label="First tempo phase"/)
  assert.match(single, /startPhase\s*=\s*'down'/)
  assert.match(whole, /startPhase\s*=\s*'down'/)
